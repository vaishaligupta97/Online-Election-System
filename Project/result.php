                ...
                <div class="form-group">
                   ...
                   <?php
                      ...
                      else
                    {
                       ...
                       if($row['time']>='25:0:0')
                       {
                         $sql = mysqli_query($conn, "SELECT electionid,title from election where electionid in(SELECT distinct Eid From counting where Eid in (select electionid from election where enddate<= ADDDATE(CURDATE(),1)))");
                       }
                        else
                        {
                          $sql = mysqli_query($conn, "SELECT electionid,title from election where electionid in(SELECT distinct Eid From counting where Eid in (select electionid from election where enddate<= CURDATE()");
                        }
